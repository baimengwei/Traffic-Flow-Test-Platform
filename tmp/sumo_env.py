...
class Intersection:
    ...
    def _update_feature(self):
        ...
        dic_feature["lane_vehicle_cnt"] = self._get_lane_vehicle_cnt(self.list_entering_lanes)
        dic_feature["lane_vehicle_cnt_been_stopped_thres01"] = self._get_lane_vehicle_cnt_been_stopped(0.1, self.list_entering_lanes)
        dic_feature["lane_vehicle_cnt_been_stopped_thres1"] = self._get_lane_vehicle_cnt_been_stopped(1, self.list_entering_lanes)
        dic_feature["lane_queue_length"] = self._get_lane_queue_length(self.list_entering_lanes)
        dic_feature["lane_vehicle_left_cnt"] = None
    ...
    def _get_lane_vehicle_cnt(self, list_lanes):
        '''
    ...
    def _get_lane_vehicle_left_cnt(self, list_lanes):

        list_lane_vehicle_left = self._get_lane_list_vehicle_left(list_lanes)
        list_lane_vehicle_left_cnt = [len(lane_vehicle_left) for lane_vehicle_left in list_lane_vehicle_left]
        return list_lane_vehicle_left_cnt
...
class SumoEnv:
    ...
    def bulk_log(self):
        ...
        for inter_ind in range(len(self.list_intersection)):
            ...
            print(feature['lane_vehicle_cnt'])
            if max(feature['lane_vehicle_cnt']) > self.dic_traffic_env_conf["VALID_THRESHOLD"]:
