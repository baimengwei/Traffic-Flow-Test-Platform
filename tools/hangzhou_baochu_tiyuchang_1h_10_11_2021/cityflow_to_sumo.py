...
def create_file_net(args):
    cmd = "netconvert --tls.ungroup-signals -n %s -e %s -x %s -i %s -o %s" % (
...
def clean_tmp_file(args):
    os.remove(str(args.sumonet).split('.')[0] + ".nod.xml")
    os.remove(str(args.sumonet).split('.')[0] + ".edg.xml")
    os.remove(str(args.sumonet).split('.')[0] + ".con.xml")
    os.remove(str(args.sumonet).split('.')[0] + ".tll.xml")
