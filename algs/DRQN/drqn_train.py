...
def drqn_train(dic_exp_conf, dic_agent_conf, dic_traffic_env_conf,
    ...
    learner = DRQNLearner(dic_exp_conf, dic_agent_conf, dic_traffic_env_conf,
                          dic_path, round_number)
    learner.learn_round()
...
def main(args):
    ...
    traffic_file_list = \
        list(dic_traffic_env_conf["TRAFFIC_CATEGORY"]["train_all"].keys())[1:4]
    ...
    for traffic_file in traffic_file_list:
        p = Process(target=pipeline, args=(args, traffic_file, drqn_train,))
        p.start()
