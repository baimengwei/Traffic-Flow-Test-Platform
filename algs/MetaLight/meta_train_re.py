...
def main(args):
    ...
    torch.manual_seed(dic_agent_conf['SEED'])
...
def metalight_train(dic_exp_conf, dic_agent_conf, _dic_traffic_env_conf,
    ...
    Args:
        ...
        _dic_traffic_env_conf:
        _dic_path:
        tasks: traffic files name(different) in this round
        batch_id: round number
    Returns:
    """
    tot_path = []
    tot_traffic_env = []
    for task in tasks:
        _dic_traffic_env_conf = copy.deepcopy(_dic_traffic_env_conf)
        _dic_path = copy.deepcopy(_dic_path)
        _dic_traffic_env_conf = \
            update_traffic_env_conf2(task,_dic_traffic_env_conf,_dic_path)
        _dic_path = \
            update_path2(task, _dic_path)

        tot_path.append(_dic_path)
        tot_traffic_env.append(_dic_traffic_env_conf)
    ...
    if batch_id == 0:
        with open(os.path.join(_dic_path['PATH_TO_MODEL'],
