...
class MultiTasksSnapshot(object):
	def __init__(self, max_size=1e3):
		"""
			all task will have same size. the max_size is very smaller
			compare to create Buffer(e.g. e6), it may be just a snapshot,
			the max_size should be (e.g. 2000)
		"""
		self.max_size = max_size
		
	def init(self, task_ids=None):
		"""
			init buffers for all tasks, return self.task_buffers based on
			Buffer object. operation based on max_size
		"""
		self.task_buffers = dict(
			[(idx, Buffer(max_size = self.max_size)) for idx in task_ids])
