...
class MQL:
    def __init__(self,
                 actor, actor_target, critic, critic_target, lr=None,
                 gamma=0.99, ptau=0.005, policy_noise=0.2, noise_clip=0.5,
                 policy_freq=2, batch_size=100, optim_method='',
                 max_action=None, max_iter_logistic=2000, beta_clip=1,
                 enable_beta_obs_cxt=False, prox_coef=1, device='cpu',
                 lam_csc=1.0, type_of_training='csc', use_ess_clipping=False,
                 use_normalized_beta=True, reset_optims=False):
        """
        Args:
            ptau: Interpolation factor in polyak averaging, soft update param
            ...
            optim_method:
            max_action:
            max_iter_logistic: decide whether to concat obs and ctx for logistic regression
            beta_clip: Range to clip beta term in CSC
            enable_beta_obs_cxt:
            prox_coef: Prox lambda
            lam_csc:  logisitc regression reg, smaller means stronger reg
        ...
        # this is used to avoid inf or nan in calculations
    ...
    def set_tasks_list(self, tasks_idx):
        """
            Keep copy of task lists, get self.train_tasks_list.
        """
        self.train_tasks_list = set(tasks_idx.copy())

    def select_action(self, obs, previous_action, previous_reward,
                      previous_obs):
        """
        Args:
            obs: one step s
            previous_action: history array a
            previous_reward:history array r
            previous_obs: history array s
        Returns:
            action based on self.actor, obs and history data.
        """
        obs = torch.FloatTensor(
            obs.reshape(1, -1)).to(self.device)
        previous_action = torch.FloatTensor(
            previous_action.reshape(1, -1)).to(self.device)
        previous_reward = torch.FloatTensor(
            previous_reward.reshape(1, -1)).to(self.device)
        previous_obs = torch.FloatTensor(
            previous_obs.reshape(1, -1)).to(self.device)
    ...
    def train_cs(self, task_id=None, snap_buffer=None,
                 train_tasks_buffer=None, adaptation_step=False):
        """
        ...
        Args:
            ...
            adaptation_step: can be True
        Returns:
            regression model and info.
        """
        ...
        if adaptation_step:
            ...
            task_bsize = int(snap_buffer.size_rb(task_id) /
                             (len(self.train_tasks_list))) + 2
            neg_tasks_ids = self.train_tasks_list
        else:
            # step 1: calculate how many samples per classes we need
            task_bsize = int(snap_buffer.size_rb(task_id) /
                             (len(self.train_tasks_list) - 1)) + 2
            neg_tasks_ids = list(
                self.train_tasks_list.difference(set([task_id])))

        # collect examples from other tasks and consider them as one class
        # view --> len(neg_tasks_ids),task_bsize, D ==> len(neg_tasks_ids) *
        # task_bsize, D
        pu, pr, px, xx = train_tasks_buffer.sample(
            task_ids=neg_tasks_ids, batch_size=task_bsize)
        neg_actions = torch.FloatTensor(pu).view(
            task_bsize * len(neg_tasks_ids), -1).to(self.device)
        neg_rewards = torch.FloatTensor(pr).view(
            task_bsize * len(neg_tasks_ids), -1).to(self.device)
        neg_obs = torch.FloatTensor(px).view(
            task_bsize * len(neg_tasks_ids), -1).to(self.device)
        neg_xx = torch.FloatTensor(xx).view(
            task_bsize * len(neg_tasks_ids), -1).to(self.device)

        # sample current task and consider it as another class
        # returns size: (task_bsize, D)
        ppu, ppr, ppx, pxx = snap_buffer.sample(
            task_ids=[task_id], batch_size=snap_buffer.size_rb(task_id))
        ...
        with torch.no_grad():
            ...
            # torch.cat ([batch_size , obs_dim], [batch_size , context_size])
            # ==> [batch_size, obs_dim + context_size ]
            if self.enable_beta_obs_cxt:
                snap_ctxt = torch.cat([pos_pxx, self.actor.get_conext_feats(
                    pos_act_rew_obs)], dim=-1).cpu().data.numpy()
                neg_ctxt = torch.cat([neg_xx, self.actor.get_conext_feats(
                    neg_act_rew_obs)], dim=-1).cpu().data.numpy()
            else:
                snap_ctxt = self.actor.get_conext_feats(
                    pos_act_rew_obs).cpu().data.numpy()
                neg_ctxt = self.actor.get_conext_feats(
                    neg_act_rew_obs).cpu().data.numpy()
        ...
        y = np.concatenate(
            (-np.ones(snap_ctxt.shape[0]), np.ones(neg_ctxt.shape[0])))

        # model params : [1 , D] where D is context_hidden
        model = logistic(max_iter=self.max_iter_logistic,
                         C=self.lam_csc).fit(x, y)
        # keep track of how good is the classifier
        prediction_score = model.score(x, y)

        info = (snap_ctxt.shape[0], neg_ctxt.shape[0], prediction_score)

        return model, info

    def update_prox_w_ess_factor(self, cs_model, x, beta=None):
        """
            from beta to get ess_factor and update self.prox_coef(lambda)
        ...
        n = x.shape[0]
        if beta is not None:
            # beta results should be same as using
            # cs_model.predict_proba(x)[:,0] if no clipping
    ...
    def get_propensity(self, cs_model, curr_pre_act_rew, curr_obs):
        ...
        with torch.no_grad():
            # batch_size X context_hidden
            if self.enable_beta_obs_cxt:
                ctxt = torch.cat([curr_obs, self.actor.get_conext_feats(
                    curr_pre_act_rew)], dim=-1).cpu().data.numpy()
            else:
                ctxt = self.actor.get_conext_feats(
                    curr_pre_act_rew).cpu().data.numpy()
        ...
        if self.use_normalized_beta:
            ...
            f_score = (d_pmax_pmin * (f_score - torch.min(f_score))) / (torch.max(
                f_score) - torch.min(f_score) + self.r_eps) + np.float32(np.min(lr_prob))
    ...
    def do_training(self, replay_buffer=None, iterations=None, csc_model=None,
                    apply_prox=False, current_batch_size=None,
                    src_task_ids=[]):
        """
        Using for MQL update phase in eq 18 or eq 19 with beta_score and lambda
        Args:
            replay_buffer: a buffer. maybe just a snap buffer from eval task.
                        or main replay buffer.
            iterations: snap_iter_nums: how many times adapt using eval task(args)
                        or main_snap_iter_nums, e.g. 100
            csc_model: None, (control beta_score generate method)
                        or csc_model which is a logistic regression model.
            apply_prox: False for using eqn (6) which is normal update.
                        or True for using eqn(18) which has penalty item. lambda
            current_batch_size: replay_buffer size.
                        or sample_mult * self.batch_size
            src_task_ids:
        Returns:
            training info
        """
        ...
        for it in range(iterations):
            ...
            if len(src_task_ids) > 0:
                x, y, u, r, d, pu, pr, px, nu, nr, nx = \
                    replay_buffer.sample_tasks(
                        task_ids=src_task_ids, batch_size=current_batch_size)
            else:
                x, y, u, r, d, pu, pr, px, nu, nr, nx = \
                    replay_buffer.sample(current_batch_size)
            ...
            if csc_model is None:
                ...
                beta_score = torch.ones(
                    (current_batch_size, 1)).to(self.device)
            else:
                # propensity_scores dim is batch_size
                beta_score, clipping_factor = \
                    self.get_propensity(csc_model, pre_act_rew, obs)
            ...
            # if done , then only use reward otherwise reward + (self.gamma *
            # target_Q)
            target_Q1, target_Q2 = self.critic_target(
                next_obs, next_action, act_rew)
            ...
            assert critic_loss_temp.shape == beta_score.shape, \
                ('shape critic_loss_temp and beta_score should be same',
                 critic_loss_temp.shape, beta_score.shape)
            ...
            if apply_prox:
                ...
                critic_prox = self.get_prox_penalty(
                    self.critic, self.ckpt['critic'])
            ...
            if it % self.policy_freq == 0:
                # Compute actor loss
                actor_loss_temp = -1 * beta_score * \
                    self.critic.Q1(
                        obs, self.actor(obs, pre_act_rew), pre_act_rew)
                actor_loss = actor_loss_temp.mean()
                actor_loss_out += actor_loss.item()

                if apply_prox:
                    # calculate proximal term
                    actor_prox = self.get_prox_penalty(
                ...
                for param, target_param in zip(
                    ...
                    target_param.data.copy_(
                        self.ptau * param.data + (1 - self.ptau) * target_param.data)
                for param, target_param in zip(
    ...
    def train_TD3(self, replay_buffer=None, iterations=None,
                  tasks_buffer=None, train_iter=0, task_id=None,
                  nums_snap_trains=5):
        """
        Args:
            replay_buffer:
            ...
            nums_snap_trains:
        Returns:
            out, None, out is a dict store the average loss of actor and critic
        ...
        for it in range(iterations):
            ...
            next_action = (self.actor_target(next_obs, act_rew) + noise)\
            ...
            # 3. Compute critic loss
            # even we picked min Q, we still need to backprob to both Qs
            # critic_loss_out is for logging.
            ...
            ########
            # Delayed policy updates
            # policy_freq e.g. 2,3,4 based on args and cmd.
            ########
            if it % self.policy_freq == 0:
                # Compute actor loss
                actor_loss = - self.critic.Q1(
                    obs, self.actor(obs, pre_act_rew), pre_act_rew).mean()
                ...
                for param, target_param in zip(
                    ...
                    target_param.data.copy_(
                        self.ptau * param.data +
                        (1 - self.ptau) * target_param.data)
    ...
    def adapt(self, train_replay_buffer=None, train_tasks_buffer=None,
              eval_task_buffer=None, task_id=None, snap_iter_nums=5,
              main_snap_iter_nums=15, sampling_style='replay', sample_mult=1):
        """
        Args:
            train_replay_buffer:
            train_tasks_buffer:
            eval_task_buffer:
            task_id:
            snap_iter_nums: how many times adapt using eval task, e.g. 10
            main_snap_iter_nums: how many times adapt using train task but with csc, e.g. 100
            sampling_style: should be replay in args.
            sample_mult: sample multipler of main_iter for adapt method, e.g. 5
        Returns:
            out( training info using eq18), out_single information(normal train)
        """
        #######
        ...
        csc_model, csc_info = \
            self.train_cs(task_id=task_id,
                          snap_buffer=eval_task_buffer,
                          train_tasks_buffer=train_tasks_buffer,
                          adaptation_step=True)

        # train td3 for a single task
        out_single = self.do_training(
            replay_buffer=eval_task_buffer.get_buffer(task_id),
    ...
    def rollback(self):
        ...
        self.critic_target.load_state_dict(
            self.critic_target_copy.state_dict())
        self.actor_optimizer.load_state_dict(
            self.actor_optimizer_copy.state_dict())
        self.critic_optimizer.load_state_dict(
            self.critic_optimizer_copy.state_dict())
    ...
    def train(self, replay_buffer=None, iterations=None, tasks_buffer=None,
              train_iter=0, task_id=None, nums_snap_trains=5):
        """
